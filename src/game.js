...
function applySkillEffect(effectKey, unit, target, ctx) {
  ...
  switch (effectKey) {
    ...
    case 'drain': {
      if (!t) break;
      const dHp = Math.max(1, Math.ceil((getEffectiveStat(u, 'int') * 0.6) - (getEffectiveStat(t, 'int') * 0.4 + getEffectiveStat(t, 'luk') * 0.3)));
      const dMp = Math.max(1, Math.ceil((getEffectiveStat(u, 'int') * 0.3) - (getEffectiveStat(t, 'int') * 0.4 + getEffectiveStat(t, 'luk') * 0.3)));
